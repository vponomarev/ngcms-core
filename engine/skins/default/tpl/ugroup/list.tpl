	...
	<tbody id="admCatList">
		...
		<tr align="center" class="contRow1">
			...
			<td>{% if (entry.flags.canEdit) %}[
				<a href="?mod=ugroup&action=editForm&id={{ entry.id }}">Edit</a>
				]{% endif %}{% if (entry.flags.canDelete) %}[
		...
		<tr>
			<td colspan="8" class="contentEdit" align="left">&nbsp; {% if (flags.canAdd) %}
				<form method="get" action="">
					<input type="hidden" name="mod" value="ugroup" /><input type="hidden" name="action"
						value="addForm" /><input type="submit" value="{{ lang['add_group'] }}" class="button" />
				</form>{% endif %}</td>
		</tr>
	</tfoot>
</table>
