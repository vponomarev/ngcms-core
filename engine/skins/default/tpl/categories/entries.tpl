...
<tr align="center" class="contRow1">
	<td>
		{% if (flags.canModify) %}
		<a href="#" onclick="categoryModifyRequest('up', {{ entry.id }});"><img
				src="{{ skins_url }}/images/up.gif" /></a>
		<a href="#" onclick="categoryModifyRequest('down', {{ entry.id }});"><img
				src="{{ skins_url }}/images/down.gif" /></a>
		{% endif %}
	</td>
	<td>
		...
		<div style="float: left;">
			{% if (flags.canView) %}
			<a href="admin.php?mod=categories&amp;action=edit&amp;catid={{ entry.id }}"
				title="ID: {{ entry.id }}">{{ entry.name }}</a>{% else %}{{ entry.name }}{% endif %}
			<br />
			<small>
	...
	<td>
		<a
			href="admin.php?mod=news&amp;category={{ entry.id }}">{% if (entry.news == 0) %}--{% else %}{{ entry.news }}{% endif %}</a>
	</td>
