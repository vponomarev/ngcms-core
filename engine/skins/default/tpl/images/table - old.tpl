	...
	<div class="tabs-content">
		<div class="clear" id="list">
			<form action="{php_self}" method="get" name="options_bar">
				...
				<dl class="fl">
					...
					<dd><select name="author" id="author">
							<option value="">- {l_all} -</option>{authorlist}
						</select></dd>
					[status]
				...
				<dl class="fl">
					...
					<dd><select name="postdate" id="postdate">
							<option selected value="">- {l_all} -</option>{dateslist}
						</select></dd>
				...
				<label class="clear w_100"><input type="checkbox"
						onclick="setCookie('img_preview',this.checked?1:0); document.location=document.location;"
						{box_preview} /> {l_show_preview}</label>
			</form>
		</div>
		...
		<form action="{php_self}?mod=images" method="post" name="imagedelete" id="delform">
			...
			<table class="table-resp table-images" id="entries">
				<thead>
					<tr>
						<th><input class="check" type="checkbox" name="master_box" title="{l_select_all}"
								onclick="check_uncheck_all(imagedelete)" /></th>
						<th>ID</th>
						<th>{l_name}</th>
						<th>{l_header.view}</th>
	...
	<div class="tabs-content" id="uploadnew">
		<div class="div-resp">
			...
			<form action="{php_self}?mod=images" method="post" enctype="multipart/form-data" name="sn">
				...
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="replace" id="flagReplace"
							value="1" /></span>
				...
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="thumb" id="flagThumb" value="1"
							{thumb_mode}{thumb_checked} /></span>
					<label for="flagThumb">{l_do_preview}</label>
				</div>
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="shadow" value="1" id="flagShadow"
							{shadow_mode}{shadow_checked} /></span>
					<label for="flagShadow">{l_do_shadow}</label>
				</div>
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="stamp" value="1" id="flagStamp"
							{stamp_mode}{stamp_checked} /></span>
						...
						<td>
							<div class="input-group">
								...
								<div class="btn btn-default btn-fileinput">
									...
									<input type="file" name="userfile[]" id="userfile[0]"
										onchange="checkImage(this, 0);" multiple />
								</div>
								<code></code>
							</div>
						</td>
		...
		<div class="div-resp">
			...
			<form action="{php_self}?mod=images" method="post" name="snup">
				...
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="replace" id="replace2"
							value="1" /></span>
				...
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="thumb" id="thumb2" value="1"
							{thumb_mode}{thumb_checked} /></span>
					<label for="thumb2">{l_do_preview}</label>
				</div>
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="shadow" id="shadow2" value="1"
							{shadow_mode}{shadow_checked} /></span>
					<label for="shadow2">{l_do_shadow}</label>
				</div>
				<div class="input-group">
					<span class="input-group-check"><input type="checkbox" name="stamp" id="stamp2" value="1"
							{stamp_mode}{stamp_checked} /></span>
		...
		<div class="clear"></div>
		<div class="sysinfo_common">Информация<div class="info" id="mfs"></div>
		</div>
		<script
			type="text/javascript">$('#mfs').html('Максимальный размер изображения: ' + formatSize({ maxSize }) + '<br />Допустимые расширения изображений: {listExt}');</script>
	</div>
</div>
