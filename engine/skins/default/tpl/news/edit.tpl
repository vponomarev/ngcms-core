				...
				<table border="0" cellspacing="1" cellpadding="0" width="100%">
					<tr>
						<td class="contentNav" align="center">
							<input type="button" onmousedown="javascript:ChangeOption('maincontent')"
								value="{{ lang.editnews['bar.maincontent'] }}" class="navbutton" />
							<input type="button" onmousedown="javascript:ChangeOption('additional')"
								value="{{ lang.editnews['bar.additional'] }}" class="navbutton" />
							<input type="button" onmousedown="javascript:ChangeOption('attaches')"
								value="{{ lang.editnews['bar.attaches'] }} ({% if (attachCount>0) %}{{ attachCount }}{% else %}{{ lang['noa'] }}{% endif %})"
								class="navbutton" />
							{% if (pluginIsActive('comments')) %}
							<input type="button" onmousedown="javascript:ChangeOption('comments')"
								value="{{ lang.editnews['bar.comments'] }} ({{ plugin.comments.count }})"
								class="navbutton" />{% endif %}
						</td>
					</tr>
					<tr>
						<td>
							...
							<div id="maincontent" style="display: block;">
								<table width="100%" cellspacing="1" cellpadding="0" border="0">
									<tr>
										...
										<td>
											<input type="text" class="important" size="79" id="newsTitle" name="title"
									...
									<tr>
										<td valign="top" colspan=3>{% if (not isBBCode) %}{{ quicktags }}
											...
											<div id="container.content.short" class="contentActive">
												<textarea style="width: 99%; padding: 1px; margin: 1px;"
													onclick="changeActive('short');" onfocus="changeActive('short');"
													name="ng_news_content_short"
													{% if (isBBCode) %}class="{{ attributBB }}"
													{% else %}id="ng_news_content_short" {% endif %} rows="10"
													tabindex="2">{{ content.short }}</textarea>
											</div>
											...
											<table cellspacing="2" cellpadding="0" width="100%">
												<tr>
													...
													<td style="width: 90%">
														<input tabindex="2" type="text" name="content_delimiter"
											...
											<div id="container.content.full" class="contentInactive">
												<textarea style="width: 99%; padding: 1px; margin: 1px;"
													onclick="changeActive('full');" onfocus="changeActive('full');"
													name="ng_news_content_full"
													{% if (isBBCode) %}class="{{ attributBB }}"
													{% else %}id="ng_news_content_full" {% endif %} rows="10"
													tabindex="2">{{ content.full }}</textarea>
											</div>
											{% else %}
											<div id="container.content" class="contentActive">
												<textarea style="width: 99%; padding: 1px; margin: 1px;"
													name="ng_news_content" {% if (isBBCode) %}class="{{ attributBB }}"
													{% else %}id="ng_news_content" {% endif %} rows="10"
													tabindex="2">{{ content.short }}</textarea>
											</div>
											{% endif %}
										</td>
									...
									<tr>
										...
										<td>
											<input type="text" name="alt_name" value="{{ alt_name }}"
												{% if flags['altname.disabled'] %}disabled="disabled"
												{% endif %}size="60" tabindex="3" />
										</td>
									</tr>
									{% if (flags.meta) %}
									<tr>
									...
									<tr>
										...
										<td>
											<textarea name="keywords" id="newsKeywords"
												cols="80">{{ keywords }}</textarea>
										</td>
									</tr>
									{% endif %}
					...
					<table border="0" cellspacing="1" cellpadding="0" width="98%">
						...
						<tr>
							<td class="contentHead">
								<img src="{{ skins_url }}/images/nav.png" hspace="8"
									alt="" />{{ lang.editnews['date.manage'] }}
							</td>
						</tr>
						<tr>
							<td class="contentEntry1">
								<table cellspacing=1 cellpadding=1 style="font: 11px verdana, sans-serif;">
									<tr>
										<td>
											<input type="checkbox" name="setdate_custom" id="setdate_custom" value="1"
												class="check"
												onclick="document.getElementById('setdate_current').checked=false;" />
										</td>
									...
									<tr>
										<td>
											<input type="checkbox" name="setdate_current" id="setdate_current" value="1"
												class="check"
												onclick="document.getElementById('setdate_custom').checked=false;" />
										</td>
										<td>
					...
					<table width="98%" cellspacing="1" cellpadding="2" border="0" id="attachFilelist">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<a style="cursor:pointer"
										onclick="ChangeOption('maincontent'); insertext('[attach#{{ entry.id }}]{{ entry.orig_name }}[/attach]','', currentInputAreaID)"
										title='{{ lang['tags.file'] }}'><img src="{{ skins_url }}/tags/attach.png"
							...
							<tr>
								<td colspan="3">&nbsp;</td>
								<td colspan="2">
									<input type="button" class="button" value="{{ lang.editnews['attach.more_rows'] }}"
										style="width: 100%;" onclick="attachAddRow();" />
								</td>
							</tr>
					</table>
				</div>
				...
				<table width="100%" cellspacing="0" cellpadding="0" border="0">
					...
					<tr>
						...
						<td>
							<div class="list">
								{{ lang['editor.author'] }}:
								<a style="font-family: Tahoma, Sans-serif;"
									href="{{ php_self }}?mod=users&amp;action=editForm&amp;id={{ authorid }}"><b>{{ author }}</b></a>
								{% if (pluginIsActive('uprofile')) %}
								<a href="{{ author_page }}" target="_blank"
									title="{{ lang.editnews['site.viewuser'] }}">
									<img src="{{ skins_url }}/images/open_new.png"
										alt="{{ lang.editnews['newpage'] }}" />
								</a>{% endif %}<br />
					...
					<tr>
						...
						<td>
							<div class="list">
								<label><input type="checkbox" name="mainpage" value="1"
										{% if (flags.mainpage) %}checked="checked" {% endif %} class="check"
										id="mainpage" {% if (flags['mainpage.disabled']) %}disabled{% endif %} />
									{{ lang.editnews['mainpage'] }}
								</label><br />
								<label><input type="checkbox" name="pinned" value="1"
										{% if (flags.pinned) %}checked="checked" {% endif %} class="check" id="pinned"
										{% if (flags['pinned.disabled']) %}disabled{% endif %} />
									{{ lang.editnews['add_pinned'] }}
								</label><br />
								<label><input type="checkbox" name="catpinned" value="1"
										{% if (flags.catpinned) %}checked="checked" {% endif %} class="check"
										id="catpinned" {% if (flags['catpinned.disabled']) %}disabled{% endif %} />
									{{ lang.editnews['add_catpinned'] }}
								</label><br />
								<label><input type="checkbox" name="favorite" value="1"
										{% if (flags.favorite) %}checked="checked" {% endif %} class="check"
										id="favorite" {% if (flags['favorite.disabled']) %}disabled{% endif %} />
									{{ lang.editnews['add_favorite'] }}
								</label><br />
								<label><input type="checkbox" name="setViews" value="1" class="check" id="setViews"
										{% if (flags['setviews.disabled']) %}disabled{% endif %} />
									{{ lang.editnews['set_views'] }}
									:</label>
								<input type="text" size="4" name="views" value="{{ views }}"
									{% if (flags['setviews.disabled']) %}disabled{% endif %} /><br />
								<label><input name="flag_HTML" type="checkbox" class="check" id="flag_HTML" value="1"
										{% if (flags.html) %}checked="checked" {% endif %}
										{% if (flags['html.disabled']) %}disabled{% endif %} />
									{{ lang.editnews['flag_html'] }}
								</label><br />
								<label><input type="checkbox" name="flag_RAW" value="1"
										{% if (flags.raw) %}checked="checked" {% endif %} class="check" id="flag_RAW"
										{% if (flags['html.disabled']) %}disabled{% endif %} />
								...
								<select name="allow_com">
									<option value="0" {{ plugin.comments['acom:0'] }}>
										{{ lang['comments:mode.disallow'] }}
									<option value="1" {{ plugin.comments['acom:1'] }}>{{ lang['comments:mode.allow'] }}
									<option value="2" {{ plugin.comments['acom:2'] }}>
										{{ lang['comments:mode.default'] }}
								</select>
								{% endif %}<br />
							</div>
						</td>
			...
			<tr>
				...
				<td class="contentEditW" align="center" valign="top">
					...
					<select size="1" disabled>
						<option>
							{% if (approve == -1) %}{{ lang['state.draft'] }}{% elseif (approve == 0) %}{{ lang['state.unpublished'] }}{% else %}{{ lang['state.published'] }}{% endif %}
						</option>
					</select> &#8594;
					<select size="1" name="approve" id="approve">
						{% if flags.can_draft %}
						<option value="-1" {% if (approve == -1) %}selected="selected" {% endif %}>
							{{ lang['state.draft'] }}</option>{% endif %}
						{% if flags.can_unpublish %}
						<option value="0" {% if (approve == 0) %}selected="selected" {% endif %}>
							{{ lang['state.unpublished'] }}</option>{% endif %}
						{% if flags.can_publish %}
						<option value="1" {% if (approve == 1) %}selected="selected" {% endif %}>
							{{ lang['state.published'] }}</option>{% endif %}
					</select>
					<input type="submit" value="{{ lang.editnews['do_editnews'] }}" accesskey="s"
						class="button" />&nbsp;{% endif %}
				</td>
				{% if flags.deleteable %}
				<td class="contentEditW" align="right" valign="top" width="150">
					<input type="button" value="{{ lang.editnews['delete'] }}"
						onClick="confirmit('{{ php_self }}?mod=news&amp;action=manage&amp;subaction=mass_delete&amp;selected_news[]={{ id }}&amp;token={{ token }}', '{{ lang.editnews['sure_del'] }}')"
						class="button" />
				</td>
				{% endif %}
			</tr>
	...
	<div id="comments" style="display: none;">
		<table border="0" cellspacing="0" cellpadding="0" width="98%">
			<tr align="center">
				...
				<td class="contentHead">
					<input type="checkbox" name="master_box" value="all"
						onclick="javascript:check_uncheck_all(commentsForm)" class="check" />
				</td>
			</tr>
			...
			<tr align="center">
				<td width="100%" colspan="6" class="contentEdit" align="center" valign="top">
					<input type="submit" value="{{ lang.editnews['comdelete'] }}"
						onClick="if (!confirm('{{ lang.editnews['sure_del_com'] }}')) {return false;}" class="button" />
				</td>
			</tr>
		</table>
	</div>
