...
<table border="0" width="100%" cellpadding="0" cellspacing="0">
	<tr>
		<td width=100% colspan="5" class="contentHead">
			...
			&#8594; {% if (flags.editMode) %}{{ lang['static_title_edit'] }}
			"{{ data.title }}"{% else %}{{ lang['static_title_add'] }}{% endif %}
		</td>
	</tr>
</table>
							...
							<div id="maincontent" style="display: block;">
								<table width="100%" border="0" cellspacing="1" cellpadding="0">
									<tr>
										...
										<td>
											<input type="text" class="important" size="79" name="title"
												value="{{ data.title }}" tabindex="1" />
										</td>
									</tr>
									<tr>
										...
										<td>{{ lang['alt_name'] }}:</td>
										<td>
											<input type="text" name="alt_name" value="{{ data.alt_name }}" size="79"
												tabindex="3" />
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<input type="text" class="important" size="79" name="url"
												readonly="readonly" value="{{ data.url }}" tabindex="1" />
									...
									<tr>
										<td valign="top" colspan=3>{% if (not isBBCode) %}{{ quicktags }}
											...
											<textarea
												style="margin-left: 0px; margin-right: 0px; margin-top: 1px; width: 99%;"
												name="content" {% if (isBBCode) %}class="{{ attributBB }}"
												{% else %}id="content" {% endif %} rows="16"
												tabindex="2">{{ data.content }}</textarea>
										</td>
									</tr>
									...
									<tr>
										...
										<td>{{ lang['keywords'] }}:</td>
										<td>
											<input type="text" name="keywords" value="{{ data.keywords }}" size="60"
												tabindex="5" />
										</td>
									</tr>
									<tr>
										...
										<td><input type="text" id="cdate" name="cdate" value="{{ data.cdate }}" />
											<input name="set_postdate" type="checkbox" value="1" />
											{{ lang['set_postdate'] }}
			...
			<td id="rightBar" width="300" valign="top">
				...
				<table width="100%" cellspacing="0" cellpadding="0" border="0">
					...
					<tr>
						...
						<td>
							<div class="list">
								<label><input type="checkbox" name="flag_published" value="1"
										{% if (not flags.canPublish) or (not flags.canUnpublish) %}disabled="disabled"
										{% endif %} {% if (data.flag_published) %}checked="checked"
										{% endif %}class="check" /> {{ lang['approve'] }}
								</label><br />
								<label><input type="checkbox" name="flag_html" value="1" class="check"
										{% if (data.flag_html) %}checked="checked" {% endif %} />
									{{ lang['flag_html'] }}
								</label><br />
								<label><input type="checkbox" name="flag_raw" value="1" class="check"
										{% if (data.flag_raw) %}checked="checked" {% endif %} /> {{ lang['flag_raw'] }}
								</label>
							</div>
						...
						<td>
							<div class="list">
								<select name="template" style="width: 200px;">{% for t in templateList %}
									<option value="{{ t }}" {% if (data.template == t) %}selected="selected"
										{% endif %}>{{ t }}{% endfor %}
								</select><br /><br />
								<label><input type="checkbox" name="flag_template_main" value="1"
										{% if (data.flag_template_main) %}checked="checked" {% endif %} class="check" />
									{{ lang['flag_main'] }}
								</label>
							</div>
						</td>
		...
		<tr align="center">
			<td width="100%" class="contentEdit" align="center" valign="top">
				...
				<input type="button" value="{{ lang['delete'] }}"
					onClick="confirmit('{{ php_self }}?mod=static&token={{ token }}&action=do_mass_delete&selected[]={{ data.id }}', '{{ lang['sure_del'] }}')"
					class="button" />{% endif %}{% else %}
