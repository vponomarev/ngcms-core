	...
	<tr>
		<td width="100%" align="right">{{ lang['per_page'] }}
			<form action="{{ php_self }}" method="get" name="options_bar">
				<input type="hidden" name="mod" value="static" /><input style="text-align: center" name="per_page"
					value="{{ per_page }}" type="text" size="3" />
	...
	<table border="0" cellspacing="0" cellpadding="0" class="content" align="center">
		<tr align="left" class="contHead">
			<td width="20">{% if (perm.modify) %}
				<input class="check" type="checkbox" name="master_box" title="{{ lang['select_all'] }}"
					onclick="javascript:check_uncheck_all(static)" />{% endif %}
			</td>
		...
		<tr align="left">
			...
			<td class="contentEntry1">
				<div style="float: left; margin: 0px;">
					{% if (perm.details) %}
					<a title="ID: {{ entry.id }}"
						href="{{ php_self }}?mod=static&amp;action=editForm&amp;id={{ entry.id }}">{% endif %}{{ entry.title }}{% if (perm.details) %}</a>{% endif %}
					<br />
	...
	<table border="0" cellspacing="0" cellpadding="0" class="content" align="center">
		<tr>
			...
			<td width="50%" class="contentEdit" align="right">{% if (perm.modify) %}
				<input type="button" value="{{ lang['addstatic'] }}"
					onclick="document.location='?mod=static&action=addForm'; return false;" class="button" />{% endif %}
				&nbsp;</td>
		</tr>
		<tr>
			<td colspan="2">&nbsp;</td>
