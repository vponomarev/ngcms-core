...
<article>
	<div class="full">
		...
		<div class="telo">
			...
			{% endif %}
			<!--/Basket-->
			{% if pluginIsActive('tags') %}{% if (p.tags.flags.haveTags) %}<b>Метки:</b> {{ tags }}{% endif %}{% endif %}
			{% if pluginIsActive('complain') %}{{ plugin_complain }}{% endif %}
			<div class="clear20"></div>
			{% if not (global.flags.isLogged) %}
			<div class="msgi"><p>Уважаемый посетитель, Вы зашли на сайт как незарегистрированный пользователь. Мы рекомендуем Вам <a href="/register/"><b>зарегистрироваться</b></a> <a href="/login/"><b>либо войти</b></a> на сайт под своим именем.</p>
