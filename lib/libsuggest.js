...
var ngSuggest = function (fieldID, params) {
    ...
    if (!this.opts.localPrefix) {
        this.opts.localPrefix   = '';                       // URL prefix for system location
    }    if (!this.opts.postURL) {
        this.opts.postURL   = this.opts.localPrefix + '/engine/rpc.php';        // URL where to send POST request (relative)
    }    if (!this.opts.iMinLen) {
        this.opts.iMinLen   = 2;                        // input: minimal len for search
    }    if (!this.opts.sId) {
        this.opts.sId       = null;                         // search: search DIV element ID
    }    if (!this.opts.sClass) {
        this.opts.sClass    = 'suggestWindow';          // search: search DIV element class
    }    if (!this.opts.stId) {
        this.opts.stId      = null;                         // search table: ID
    }    if (!this.opts.stClass) {
        this.opts.stClass   = 'suggestBlock';           // search table: class for search table
    }    if (!this.opts.stRClass) {
        this.opts.stRClass  = 'suggestRowNormal';       // search table: class for normal row
    }    if (!this.opts.stRHClass) {
        this.opts.stRHClass     = 'suggestRowHighlight';    // search table: class for HIGHLIGHTED row
    }    if (!this.opts.stCols) {
        this.opts.stCols    = 1;                        // number of columns in returning result
    }    if (!this.opts.stColsClass) {
        this.opts.stColsClass = [];                         // list of classes (1 by one) that should be applied to cols
    }    if (!this.opts.stColsHLR) {
        this.opts.stColsHLR     = [];                       // list of flags: do we need highlighing for this col
    }    if (!this.opts.hlr) {
        this.opts.hlr       = false;                    // should we manually HighLight Results
    }    if (!this.opts.reqMethodName) {
        this.opts.reqMethodName = 'dumb.ping';      // AJAX RPC Request method name
    }
    if (!this.opts.lId) {
        this.opts.lId       = null;                         // ID of loading layer
    }    if (!this.opts.delay) {
        this.opts.delay         = 500;                      // Delay before making AJAX request (ms)
    }
    if (!this.opts.cId) {
        this.opts.cId       = null;                         // `CLOSE SUGGEST` element ID
    }    if (!this.opts.cClass) {
        this.opts.cClass    = 'suggestClose';           // `CLOSE SUGGEST` class
    }
    if (!this.opts.listDelimiter) {
        this.opts.listDelimiter     = null;                     // Delimiter for list of values. Will be used if specified, AJAX query will be made for current edited value
    }    if (!this.opts.columnReturn) {
        this.opts.columnReturn = 0;                     // Number of column to return
    }
    ...
    //this.searchDIV.style.top	= (this.DOMelementPosY(this.field)+this.field.clientHeight+4)+'px';
        ...
        this.field.onkeypress   = function (event) {
            return pointer.onKeyPress(event); }
    this.field.onkeyup      = function (event) {
        return pointer.onKeyUp(event); }
    ...
    if (this.closeElement) {
        this.closeElement.onclick = function () {
            pointer.suggestShow(false); return false; }
    }
...
ngSuggest.prototype.focusField = function () {
    this.field.focus(); }
...
ngSuggest.prototype.suggestSearch = function (text) {
    ...
    this.timeoutID = setTimeout(function () {
        pointer.callAJAX() }, this.opts.delay);
}
        ...
        // First - check error state
            if (!resTX['status']) {
                ...
                for (var i = 0; i < sReturn.length; i++) {
                    ...
                    r.onclick = function () {
                        pointer.setValue(this.dataOutput); pointer.focusField(); pointer.suggestShow(false); }
                    r.onmouseover = function () {
                        pointer.highlightSet(this.dataId); }
                    ...
                    for (var fNo = 0; fNo < pointer.opts.stCols; fNo++) {
                        ...
                        if (pointer.opts.stColsHLR[fNo]) {
                            ...
                            var rV = sReturn[i][fNo].replace(rE, function (x) {
                                return '<b>'+x+'</b>'; });
                            d.innerHTML = rV;
                        } else {
